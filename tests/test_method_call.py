...
class TestMethodCall(TestCase):
    def test_simple_call(self):
        self.assertTranslated("$var.to_i()", "to_i(get(variables, 'var'))")

    def test_call_with_arguments(self):
        self.assertTranslated("$var.to_i(16)", "to_i(get(variables, 'var'), 16)")
        self.assertTranslated(
            "$var.map_join('')", "map_join(get(variables, 'var'), '')"
        )

    def test_accessing_attribute(self):
        self.assertTranslated(
            "$cdr.Custom-Attr", "get(get(variables, 'cdr'), 'Custom-Attr')"
        )

    def test_multiple_attributes(self):
        self.assertTranslated(
            "$cdr.Attr:1.Attr:2", "get(get(get(variables, 'cdr'), 'Attr:1'), 'Attr:2')"
        )

    def test_calls_as_arguments(self):
        self.assertTranslated(
            "$cdr.to_i($sixteen.upper())",
            "to_i(get(variables, 'cdr'), upper(get(variables, 'sixteen')))",
        )

        self.assertTranslated(
            "$cdr.to_i($var.sixteen, $var2.upper())",
            "to_i(get(variables, 'cdr'), get(get(variables, 'var'), 'sixteen'), "
            + "upper(get(variables, 'var2')))",
        )

    def test_expressions_as_arguments(self):
        self.assertTranslated("$var.to_i(8 + 8)", "to_i(get(variables, 'var'), 8 + 8)")

    def test_mixed_calls(self):
        self.assertTranslated(
            "$cdr.to_i(16).to_string().size",
            "get(to_string(to_i(get(variables, 'cdr'), 16)), 'size')",
        )
    ...
    def test_safe_navigation(self):
        self.assertTranslated(
            "$var?.FOO?.Maybe",
            "safe_get(safe_get(get(variables, 'var'), 'FOO'), 'Maybe')",
        )

        self.assertTranslated(
            "$var?.something(123)", "safe_call(get(variables, 'var'), something, 123)"
        )

        self.assertTranslated(
            "$var?.like?(123)", "safe_call(get(variables, 'var'), is_like, 123)"
        )
